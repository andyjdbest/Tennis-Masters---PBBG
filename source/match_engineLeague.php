...
switch ($country) {
	case 1:
		$today = date('Y-m-d H:i:s', strtotime('today' . '20 hours'));
		break;
	case 2:
		$today = date('Y-m-d H:i:s', strtotime('today' . '11 hours'));
		break;
	case 3:
		$today = date('Y-m-d H:i:s', strtotime('today' . '09 hours'));
		break;
	case 4:
		$today = date('Y-m-d H:i:s', strtotime('today' . '04 hours'));
		break;
}
$today = "2010-10-04 11:00:00";

// FETCH FIXTURES FOR THE DAY TO BE SIMMED
$fixtures = array();
$select = mysql_query("SELECT id_fixture,id_team1,id_team2,id_stadium,league_pos FROM fixtures 
		JOIN league AS l ON l.idleague=fixtures.id_league 
		WHERE round_date = '$today' AND fixture_type = 1 AND (id_league =$league OR id_league = $nleague)");
...
if (isset($fixtures)) {
	...
	$in = 0;
		...
		for ($i=0;$i <3;$i++){
			...
			// INSERT INTO MATCH ORDERS THE UPDATED ORDERS
			...
			if ($homeScore > $awayScore) {
				...
				$updateHomeLeague = "UPDATE league_table SET played = played + 1, won = won + 1, points = points + 4, pf = pf + $homeScore, pa = pa + $awayScore
							WHERE id_team = '$fixture[1]' AND season = '$season'";
				...
				$updateAwayLeague = "UPDATE league_table SET played = played + 1, lost = lost + 1, pf = pf + $awayScore, pa = pa + $homeScore
							WHERE id_team = '$fixture[2]' AND season = '$season'";
				if (!mysql_query($updateAwayLeague)) {
				//failure
					echo mysql_error();
				}
				
				//UPDATE FANS FOR HOME ACADEMY WINNING
				$new_fans = mt_rand(1,10);
				$updateHomeFans = "UPDATE academy SET fans = (fans + $new_fans), fan_move = 1 WHERE id_team = {$fixture[1]}";
				if (!mysql_query($updateHomeFans)) {
				//failure
					echo mysql_error();
				}
				//UPDATE FANS FOR AWAY ACADEMY LOSING
				$new_fans = mt_rand(1,5);
				$updateAwayFans = "UPDATE academy SET fans = (fans - $new_fans), fan_move = -1 WHERE id_team = {$fixture[2]}";
				if (!mysql_query($updateAwayFans)) {
				//failure
					echo mysql_error();
				}
			}
				...
				//UPDATE FANS FOR HOME ACADEMY LOSING
				$new_fans = mt_rand(1,5);
				$updateHomeFans = "UPDATE academy SET fans = (fans - $new_fans), fan_move = -1 WHERE id_team = {$fixture[1]}";
				if (!mysql_query($updateHomeFans)) {
				//failure
				...
				//UPDATE FANS FOR AWAY ACADEMY WINNING
				$new_fans = mt_rand(1,10);
				$updateAwayFans = "UPDATE academy SET fans = (fans + $new_fans), fan_move = 1 WHERE id_team = {$fixture[2]}";
				if (!mysql_query($updateAwayFans)) {
				//failure
			...
			//FANS BONUS
			$query = mysql_query("SELECT fans from academy WHERE id_team = {$fixture[1]}");
			$fan_row = mysql_fetch_row($query);
			
			$base = $attend[$fixture[4]][0];
			if ($league[0][0] == 0) {
				$tkt = $base * mt_rand(1,2) + $fan_row[0] * mt_rand(1,5);
			}
			else {
				$tkt = ($base + ($base/($league[0][0] + $league[1][0])))  * mt_rand(1,2) + $fan_row[0] * mt_rand(1,5);
			}
	...
	while ($setWon == 0) {
		...
		if ($server[4] == 6 && $receiver[4] == 6) {
			...
			while ($tieBreak == 1) {
				//echo 'TieBreak' . "<br />";
				//check for winner
					if ($serverPoints > 6 && ($receiverPoints < 6 || ($serverPoints - $receiverPoints) > 1)) {
						$server[4] += 1;
						$server[5] += 1;
						$setWon = 1;	
						$tieBreak = 2;
						echo "FREE $server[0] = $server[4] $serverPoints vs $receiverPoints $receiver[0] = $receiver[4] <br />";
					...
					else if ($receiverPoints > 6 && ($serverPoints < 6 || ($receiverPoints - $serverPoints) > 1)) {
						$receiver[4] += 1;
						$receiver[5] += 1;
						$setWon = 1;	
						$tieBreak = 2;
						...
						{
									// [ERH]
									// Tie-breaks use a different logic to the remainder of the match engine:
									// Because the server changes every couple of points it's not as easy to keep track
									// of the score.  To compensate, we swap the server and receiver points at every
									// change of serve
									$tempPoints = $serverPoints;
									$serverPoints = $receiverPoints;
									$receiverPoints = $tempPoints;
			...
			if ($homePlayerName === $server[0])
				$commentary[$c] = $commentary[$c] . "Score" . "}$home*$setResult*$away]}}";
			else
				$commentary[$c] = $commentary[$c] . "Score" . "}$home*$setResult*$away]}}";
		}
		else {
...
$commentary[$c] = $commentary[$c] . "Game Set Match}}";
...
function getResult($serveskill, $receiveskill, $serverrally, $receiverrally, $serverStats, $receiverStats, $sID, $hID){
    ...
    // CHECK FOR FAULT (40%)
    $checkServe = random_number('FLOAT', 0, $value);
